...
use pmemblk_sys::PMEMBLK_MIN_BLK;
    ...
    pub fn open_or_create(&self, block_pool_sets_folder_path: &Path, file_name: &str) -> BlockPool {
        ...
        if likely(pool_set_file_path.exists()) {
            ...
            let pool_size = match self.pool_size {
                ...
                Some(pool_size) => {
                    assert!(
                        pool_size >= PMEMBLK_MIN_POOL,
                        "pool_size '{}' is smaller than PMEMBLK_MIN_POOL '{}'",
                        pool_size,
                        PMEMBLK_MIN_POOL
                    );
                    pool_size
            ...
            BlockPool::create(&pool_set_file_path, block_size, pool_size, self.permissions)
                .expect("Could not create BlockPool")
        }
    }
}
