    ...
    pub fn open_or_create(&self, log_pool_sets_folder_path: &Path, file_name: &str) -> LogPool {
        ...
        if likely(pool_set_file_path.exists()) {
            ...
            let pool_size = match self.pool_size {
                ...
                Some(pool_size) => {
                    assert!(
                        pool_size >= PMEMLOG_MIN_POOL,
                        "pool_size '{}' is smaller than PMEMLOG_MIN_POOL '{}'",
                        pool_size,
                        PMEMLOG_MIN_POOL
                    );
                    pool_size
            ...
            LogPool::create(&pool_set_file_path, pool_size, self.permissions)
                .expect("Could not create LogPool")
        }
    }
}
