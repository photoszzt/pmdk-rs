...
trait PMEMlogpoolExt {
    ...
    const STOP_WALKING: WalkCallbackResult = 0;

    /// Constant to continue walking log.
    const CONTINUE_WALKING: WalkCallbackResult = 1;
    ...
    fn walk(
        self,
        chunk_size: usize,
        for_each_chunk_callback: ForEachChunkCallback,
...
impl PMEMlogpoolExt for *mut PMEMlogpool {
    ...
    fn append_atomically(self, buffer: *const c_void, count: usize) -> Result<(), AppendError> {
        ...
        if likely(result != 0) {
            ...
            panic!(
                "pmemlog_append() return a value which wasn't -1 or 0, but '{}'",
                result
            );
        } else {
            match errno().0
    ...
    fn append_vector_atomically(self, buffer: *const iovec, count: u31) -> Result<(), AppendError> {
        ...
        debug_assert!(
            count <= 2_147_483_648,
            "count '{}' must be less than or equal to 2^31",
            count
        );
        ...
        if likely(result != 0) {
            ...
            panic!(
                "pmemlog_appendv() return a value which wasn't -1 or 0, but '{}'",
                result
            );
        } else {
            match errno().0
