...
impl<'a, T: Persistable> ConditionVariable<'a, T> {
    ...
    fn new(object_pool: *mut PMEMobjpool, condition_variable: *mut PMEMcond) -> Self {
        ...
        debug_assert!(
            condition_variable.is_not_null(),
            "condition_variable is null"
        );
    ...
    pub fn timed_wait(
        &self,
        locked_mutex: MutexUnlock<'a, T>,
        absolute_time_out: &timespec,
    ) -> (MutexUnlock<'a, T>, bool) {
        let result = unsafe {
    ...
    pub fn signal_all(&self, locked_mutex: MutexUnlock<'a, T>) {
        Self::signal(
            unsafe { pmemobj_cond_broadcast(self.object_pool, self.condition_variable) },
            locked_mutex,
        );
    }
    ...
    pub fn signal_one(&self, locked_mutex: MutexUnlock<'a, T>) {
        Self::signal(
            unsafe { pmemobj_cond_signal(self.object_pool, self.condition_variable) },
            locked_mutex,
        );
    }
