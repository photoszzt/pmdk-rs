...
impl Default for ObjectPoolConfiguration {
    ...
    fn default() -> Self {
        Self {
            pool_size: None,
            permissions: configuration::Configuration::DEFAULT_PERMISSIONS_FOR_POOL_SETS,
            layout_name: None,
...
impl ObjectPoolConfiguration {
    ...
    pub fn open_or_create(
        &self,
        object_pool_sets_folder_path: &Path,
        file_name: &str,
    ) -> ObjectPool {
        ...
        assert!(
            layout_name.len() + 1 <= PMEMOBJ_MAX_LAYOUT,
            "layout_name '{}' can not be larger than PMEMOBJ_MAX_LAYOUT '{}' - 1",
            layout_name,
            PMEMOBJ_MAX_LAYOUT
        );
        ...
        let object_pool = if likely(pool_set_file_path.exists()) {
            assert!(
                pool_set_file_path.is_file(),
                "pool_set_file_path '{:?}' is not a file",
                pool_set_file_path
            );
            ...
            let pool_size = match self.pool_size {
                ...
                Some(pool_size) => {
                    assert!(
                        ...
                        "pool_size '{}' is smaller than PMEMOBJ_MIN_POOL '{}'",
                        pool_size,
                        PMEMOBJ_MIN_POOL
                    );
                    pool_size
            ...
            .expect("Could not create ObjectPool")
        };

        object_pool.set_transaction_debug_skip_expensive_checks(self.skip_expensive_debug_checks);
        object_pool.set_transaction_cache_size_and_threshold(
            self.transaction_cache_size,
            self.transaction_cache_threshold,
        );
        object_pool
    }
}
