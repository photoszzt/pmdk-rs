...
impl<'a, T: Persistable> ReadWriteLock<'a, T> {
    ...
    pub fn timed_read(self, absolute_time_out: &timespec) -> Option<ReadLockUnlock<'a, T>> {
        let result = unsafe {
            pmemobj_rwlock_timedwrlock(self.object_pool, self.read_write_lock, absolute_time_out)
        };
        if likely(result == 0) {
            return Some(ReadLockUnlock(self));
        }
        ...
        match result {
            ...
            EDEADLK => panic!("Deadlock"),
            EAGAIN => panic!("EAGAIN; too many locks of the same lock in this thread"),
            EINVAL => panic!("object_pool or read_write_lock was null or read_write_lock
                             was invalid or absolute_time_out is out-of-range (none of these things should occur)"),
            _ => panic!("Unexpected error '{}'", result),
        }
    }
    ...
    pub fn write_in_transaction(self, transaction: Transaction) {
        let result = unsafe {
            pmemobj_tx_lock(
                pobj_tx_param_TX_PARAM_RWLOCK,
                self.read_write_lock as *mut c_void,
            )
        };
        if likely(result == 0) {
            return;
    ...
    pub fn timed_write(self, absolute_time_out: &timespec) -> Option<WriteLockUnlock<'a, T>> {
        ...
        match result {
            ...
            EDEADLK => panic!("Deadlock"),
            EAGAIN => panic!("EAGAIN; too many locks of the same lock in this thread"),
            EINVAL => panic!("object_pool or read_write_lock was null or read_write_lock was
                             invalid or absolute_time_out is out-of-range (none of these things should occur)"),
            _ => panic!("Unexpected error '{}'", result),
        }
    }
