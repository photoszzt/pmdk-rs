...
pub trait ObjectPoolPathExt {
    ...
    #[inline(always)]
    fn validate_object_pool_is_consistent(
        &self,
    ...
    #[inline(always)]
    fn create_object_pool(
        &self,
        layout_name: Option<&str>,
        pool_size: usize,
        mode: mode_t,
    ) -> Result<*mut PMEMobjpool, PmdkError>;
}
...
fn layout_as_raw_pointer(layout_name: Option<&str>) -> *const c_char {
    if let Some(layout_name) = layout_name {
        debug_assert!(
            layout_name.len() + 1 <= PMEMOBJ_MAX_ALLOC_SIZE as usize,
            "layout_name length '{}' + 1 is greater than PMEMOBJ_MAX_ALLOC_SIZE '{}'",
            layout_name.len(),
