...
impl<T: Persistable> Ord for PersistentObject<T> {
    ...
    fn cmp(&self, other: &PersistentObject<T>) -> Ordering {
        ...
        our_oid
            .pool_uuid_lo
            .cmp(&other_oid.pool_uuid_lo)
            .then_with(|| our_oid.off.cmp(&other_oid.off))
    }
}
...
impl<T: Persistable> Iterator for PersistentObject<T> {
    ...
    fn next(&mut self) -> Option<PersistentObject<T>> {
        loop {
            ...
            if likely(next.type_number() == T::TYPE_NUMBER) {
...
impl<T: Persistable> OID for PersistentObject<T> {
    ...
    fn object_pool(&self) -> *mut PMEMobjpool {
        ...
        debug_assert!(
            object_pool.is_not_null(),
            "How is the object_pool null for an allocated object?"
        );
    ...
    fn address(&self) -> *mut c_void {
        ...
        debug_assert!(
            address.is_not_null(),
            "How is the address null for an allocated object?"
        );
        address
    }
}
...
where
    T: ReadWriteLockablePersistable,
{
    /// Obtain a read lock.
    #[inline(always)]
    ...
    pub fn timed_read<'a>(
        &'a mut self,
        absolute_time_out: &timespec,
    ) -> Option<ReadLockUnlock<'a, T>> {
    ...
    #[inline(always)]
    pub fn timed_write<'a>(
        &'a mut self,
        absolute_time_out: &timespec,
    ) -> Option<WriteLockUnlock<'a, T>> {
    ...
    T: MutexLockablePersistable,
{
    ...
    #[inline(always)]
    pub fn timed_mutex<'a>(
        &'a mut self,
        absolute_time_out: &timespec,
    ) -> Option<MutexUnlock<'a, T>> {
...
impl<T: Persistable> PersistentObject<T>
where
    T: ConditionVariableMutexLockablePersistable,
{
    /// Obtain a mutex lock with a condition variable.
    #[inline(always)]
    pub fn mutex_with_condition_variable<'a>(
        &'a mut self,
    ) -> (MutexUnlock<'a, T>, ConditionVariable<'a, T>) {
    ...
    #[inline(always)]
    pub fn try_mutex_with_condition_variable<'a>(
        &'a mut self,
    ) -> Option<(MutexUnlock<'a, T>, ConditionVariable<'a, T>)> {
        let (mutex_lock, condition_variable) = self.mutex_lock_with_condition_variable();

        mutex_lock
            .try_mutex()
            .map(|mutex_unlock| (mutex_unlock, condition_variable))
    }

    /// Obtain a mutex lock with a condition variable. Time out if the mutex lock is not obtained in `absolute_time_out`.
    #[inline(always)]
    pub fn timed_mutex_with_condition_variable<'a>(
        &'a mut self,
        absolute_time_out: &timespec,
    ) -> Option<(MutexUnlock<'a, T>, ConditionVariable<'a, T>)> {
        let (mutex_lock, condition_variable) = self.mutex_lock_with_condition_variable();
        ...
        mutex_lock
            .timed_mutex(absolute_time_out)
            .map(|mutex_unlock| (mutex_unlock, condition_variable))
    }

    #[inline(always)]
    fn mutex_lock_with_condition_variable<'a>(
        &'a mut self,
    ) -> (MutexLock<'a, T>, ConditionVariable<'a, T>) {
        let object_pool = self.object_pool();
        let object = self.deref_mut();
...
fn size<T: Persistable>() -> size_t {
    ...
    debug_assert!(
        size <= PMEMOBJ_MAX_ALLOC_SIZE,
        "size '{}' exceeds PMEMOBJ_MAX_ALLOC_SIZE '{}'",
        size,
...
impl<T: Persistable> PersistentObject<T> {
    ...
    pub fn object_pool(&self) -> *mut PMEMobjpool {
        ...
        debug_assert!(
            object_pool.is_not_null(),
            "This object does not have a valid OID"
        );
    ...
    #[inline(always)]
    pub fn allocate_uninitialized_and_construct_object_root_object(
        &mut self,
        object_pool: *mut PMEMobjpool,
        arguments: &mut T::Arguments,
    ) -> Result<(), PmdkError> {
        debug_assert!(
            T::TYPE_NUMBER == 0,
            "This is not a root type, ie type number is '{}'"
        );

        #[inline(always)]
        fn allocate<T: Persistable>(
            object_pool: *mut PMEMobjpool,
            oid_pointer: &mut PMEMoid,
        ...
        Self::allocate_uninitialized_and_construct_object_internal(
            object_pool,
            &mut self.oid,
            allocate::<T>,
            arguments,
    ...
    /// At this point, self.oid can be garbage; it might also point to an existing object which hasn't been free'd
    #[inline(always)]
    pub fn allocate_uninitialized_and_construct_object(
        &mut self,
        object_pool: *mut PMEMobjpool,
        arguments: &mut T::Arguments,
    ) -> Result<(), PmdkError> {
    ...
    #[inline(always)]
    fn allocate_uninitialized_and_construct_object_internal<
        A: FnOnce(*mut PMEMobjpool, &mut PMEMoid, pmemobj_constr, *mut c_void) -> bool,
    >(
        object_pool: *mut PMEMobjpool,
        oid: &mut PMEMoid,
        allocate: A,
        ...
        #[thread_local]
        static mut CAPTURED_PANIC: Option<Box<Any + Send + 'static>> = None;

        unsafe extern "C" fn constructor<T: Persistable>(
            object_pool: *mut PMEMobjpool,
            ptr: *mut c_void,
            arg: *mut c_void,
        ) -> c_int {
            ...
            match result {
                ...
                Err(panic_payload) => {
                    CAPTURED_PANIC = Some(panic_payload);
                    -1
                }
            }
        }
        ...
        if unlikely(allocate(
            ...
            oid,
            Some(constructor::<T>),
            arguments as *mut _ as *mut _,
        )) {
            let os_error_number = errno().0;
            match os_error_number {
                ECANCELED => {
                    if let Some(captured_panic) = unsafe { replace(&mut CAPTURED_PANIC, None) } {
                ...
                _ => {
                    debug_assert!(
                        unsafe { CAPTURED_PANIC.is_none() },
                        "CapturedPanic was set and error was '{}'",
                        os_error_number
                    );
            ...
            debug_assert!(
                unsafe { CAPTURED_PANIC.is_none() },
                "CapturedPanic was set yet result was 0 (Ok)"
            );
    ...
    #[inline(always)]
    pub fn allocate_uninitialized_and_construct_object_in_transaction(
        &mut self,
        transaction: Transaction,
        object_pool: *mut PMEMobjpool,
        arguments: &mut T::Arguments,
    ) -> Result<(), PmdkError> {
        self.construct_in_transaction(object_pool, arguments, unsafe {
            pmemobj_tx_alloc(size::<T>(), T::TYPE_NUMBER)
        })
    }
    ...
    #[allow(unused_variables)]
    #[inline(always)]
    pub fn allocate_uninitialized_and_construct_object_in_transaction_without_flush(
        &mut self,
        transaction: Transaction,
        object_pool: *mut PMEMobjpool,
        arguments: &mut T::Arguments,
        ...
        self.construct_in_transaction(object_pool, arguments, unsafe {
            pmemobj_tx_xalloc(size::<T>(), T::TYPE_NUMBER, POBJ_XALLOC_NO_FLUSH)
        })
    }
    ...
    #[allow(unused_variables)]
    #[inline(always)]
    pub fn add_range_snapshot_in_transaction(
        &self,
        transaction: Transaction,
        offset: u64,
        size: size_t,
        ...
        debug_assert!(!self.oid.is_null(), "oid is null");
        debug_assert!(
            offset + size as u64 <= T::size() as u64,
            "offset '{}' + size '{}' is bigger than our size '{}'",
            offset,
            size,
            T::size()
        );
    ...
    #[allow(unused_variables)]
    #[inline(always)]
    pub fn add_range_snapshot_in_transaction_without_flush(
        &self,
        transaction: Transaction,
        offset: u64,
        size: size_t,
        ...
        Self::failure_in_transaction(unsafe {
            pmemobj_tx_xadd_range(self.oid, offset, size, POBJ_XADD_NO_FLUSH)
        })
    }
    ...
    #[inline(always)]
    pub fn add_self_to_transaction_without_flush(
        &self,
        transaction: Transaction,
    ) -> Result<(), PmdkError> {
...
/// add field to transaction macro
#[macro_export]
macro_rules! add_field_to_transaction {
    ($self: ident, $transaction: ident, $selfType: ty, $field: ident) => {{
        $self.add_range_snapshot_in_transaction(
            offset_of!($selfType, $field),
            ::std::mem::size_of_val(&$self.$field),
        )
    }};
}

/// add field to transaction without flushing the cache macro
#[macro_export]
macro_rules! add_field_to_transaction_without_flush {
    ($self: ident, $transaction: ident, $selfType: ty, $field: ident) => {{
        $self.add_range_snapshot_in_transaction_without_flush(
            offset_of!($selfType, $field),
            ::std::mem::size_of_val(&$self.$field),
        )
    }};
...
/// add contiguous fields to transaction macro
#[macro_export]
macro_rules! add_contiguous_fields_to_transaction {
    ($self: ident, $transaction: ident, $selfType: ty, $fromInclusiveField: ident, $toInclusiveField: ident) => {{
        $self.add_range_snapshot_in_transaction(
            offset_of!($selfType, $fromInclusiveField),
            (::std::mem::size_of_val(&$self.$toInclusiveField)
                + offset_of!($selfType, $toInclusiveField))
                - offset_of!($selfType, $fromInclusiveField),
        )
    }};
}

/// add contiguous fields to transaction without flush macro
#[macro_export]
