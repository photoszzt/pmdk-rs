...
impl ObjectPoolsConfiguration {
    ...
    pub fn open(&self, pools_folder_path: &Path) -> HashMap<String, ObjectPool> {
        ...
        assert!(
            object_pool_sets_folder_path.is_dir(),
            "object_pool_sets_folder_path '{:?}' is not a folder",
            object_pool_sets_folder_path
        );

        ObjectPool::set_prefault_at_create(
            self.prefault_object_pool_memory_to_improve_performance_at_create,
        );
        ObjectPool::set_prefault_at_open(
            self.prefault_object_pool_memory_to_improve_performance_at_open,
        );

        self.object_pool_configurations
            .iter()
            .map(|(file_name, object_pool_configuration)| {
                (
                    file_name.to_string(),
                    object_pool_configuration
                        .open_or_create(&object_pool_sets_folder_path, file_name),
                )
            })
            .collect()
    }
