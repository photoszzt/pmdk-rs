...
impl Persistable for root {
    const TYPE_NUMBER: TypeNumber = 0;
    ...
    unsafe fn initialize(
        ...
        instance
            .node
            .allocate_uninitialized_and_construct_object(object_pool, &mut ())
            .expect("Allocation failed for node");
    }
}
...
impl Persistable for node {
    const TYPE_NUMBER: TypeNumber = 1;
    ...
    unsafe fn initialize(
        ...
        (&mut instance.read_write_lock as *mut PMEMrwlock).zero(object_pool);
        (&mut instance.mutex_lock as *mut PMEMmutex).zero(object_pool);
        (&mut instance.condition_variable as *mut PMEMcond).zero(object_pool);

        instance
            .next
            .allocate_uninitialized_and_construct_object(object_pool, &mut ())
            .expect("Allocation failed for next");
        instance
            .foo
            .allocate_uninitialized_and_construct_object(object_pool, &mut (11))
            .expect("Allocation failed for foo");
        instance.data = 0;
    }
}
...
impl Persistable for foo {
    const TYPE_NUMBER: TypeNumber = 2;
