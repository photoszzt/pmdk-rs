...
impl<'a, T: Persistable> MutexLock<'a, T> {
    ...
    pub fn mutex_in_transaction(self, transaction: Transaction) {
        let result =
            unsafe { pmemobj_tx_lock(pobj_tx_param_TX_PARAM_MUTEX, self.mutex as *mut c_void) };
        if likely(result == 0) {
            return;
        }
    ...
    pub fn timed_mutex(self, absolute_time_out: &timespec) -> Option<MutexUnlock<'a, T>> {
        let result =
            unsafe { pmemobj_mutex_timedlock(self.object_pool, self.mutex, absolute_time_out) };
        if likely(result == 0) {
            return Some(MutexUnlock(self));
        }
