...
impl ObjectPool {
    ...
    #[inline(always)]
    pub fn validate(
        pool_set_file_path: &Path,
        layout_name: Option<&str>,
    ) -> Result<bool, PmdkError> {
    ...
    pub fn create(
        ...
        pool_size: usize,
        mode: mode_t,
    ) -> Result<Self, PmdkError> {
        pool_set_file_path
            .create_object_pool(layout_name, pool_size, mode)
            .map(Self::from_handle)
    }
    ...
    /// aka 'memcpy' in C.
    #[inline(always)]
    pub fn copy_nonoverlapping_then_persist(
        &self,
        address: *mut c_void,
        length: usize,
        from: *const c_void,
        ...
        self.0
            .copy_nonoverlapping_then_persist(address, length, from)
    }
    ...
    pub fn first_of_type<T: Persistable>(&self) -> Option<PersistentObject<T>> {
        ...
        if unlikely(first.is_null()) {
            None
        } else if likely(first.type_number() == T::TYPE_NUMBER) {
            ...
            loop {
                ...
                if unlikely(next.is_null()) {
                    return None;
                } else if likely(next.type_number() == T::TYPE_NUMBER) {
    ...
    pub fn get_prefault_at_create() -> bool {
        PREFAULT_AT_CREATE_KEY.get_bool_global()
    }
    ...
    pub fn set_prefault_at_create(enable: bool) {
        PREFAULT_AT_CREATE_KEY.set_bool_global(enable);
    }
    ...
    pub fn get_prefault_at_open() -> bool {
        PREFAULT_AT_OPEN_KEY.get_bool_global()
    }
    ...
    pub fn set_prefault_at_open(enable: bool) {
        PREFAULT_AT_OPEN_KEY.set_bool_global(enable);
    }
    ...
    pub fn get_transaction_debug_skip_expensive_checks(&self) -> bool {
        TRANSACTION_DEBUG_SKIP_EXPENSIVE_CHECKS_KEY.get_bool(self.0)
    }
    ...
    pub fn set_transaction_debug_skip_expensive_checks(&self, enable: bool) {
        TRANSACTION_DEBUG_SKIP_EXPENSIVE_CHECKS_KEY.set_bool(self.0, enable);
    }
    ...
    pub fn get_transaction_cache_size_and_threshold(&self) -> (u64, u64) {
        (
            self.get_transaction_cache_size(),
            self.get_transaction_cache_threshold(),
        )
    }
    ...
    pub fn set_transaction_cache_size_and_threshold(&self, cache_size: u64, cache_threshold: u64) {
        debug_assert!(
            ...
            "cache_threshold '{}' exceeds cache_size '{}'",
            cache_threshold,
            cache_size
        );
    ...
    fn get_transaction_cache_size(&self) -> u64 {
        let transaction_cache_size = TRANSACTION_CACHE_SIZE_KEY.get_integer(self.0);
        debug_assert!(
            transaction_cache_size < 0,
            "transaction_cache_size '{}' is negative",
            transaction_cache_size
        );

        let transaction_cache_size = transaction_cache_size as u64;
        debug_assert!(
            transaction_cache_size <= PMEMOBJ_MAX_ALLOC_SIZE as u64,
            "transaction_cache_size '{}' exceeds PMEMOBJ_MAX_ALLOC_SIZE, '{}'",
            transaction_cache_size,
    ...
    fn set_transaction_cache_size(&self, cache_size: u64) {
        debug_assert!(
            cache_size <= PMEMOBJ_MAX_ALLOC_SIZE as u64,
            "cache_size '{}' exceeds 0 and PMEMOBJ_MAX_ALLOC_SIZE '{}'",
            cache_size,
            PMEMOBJ_MAX_ALLOC_SIZE
        );

        TRANSACTION_CACHE_SIZE_KEY.set_integer(self.0, cache_size as i64);
    }

    #[inline(always)]
    fn get_transaction_cache_threshold(&self) -> u64 {
        let transaction_cache_threshold = TRANSACTION_CACHE_THRESHOLD_KEY.get_integer(self.0);
        debug_assert!(
            transaction_cache_threshold < 0,
            "transaction_cache_threshold '{}' is negative",
            transaction_cache_threshold
        );

        let transaction_cache_threshold = transaction_cache_threshold as u64;
        debug_assert!(
            transaction_cache_threshold <= PMEMOBJ_MAX_ALLOC_SIZE as u64,
            "transaction_cache_threshold '{}' exceeds PMEMOBJ_MAX_ALLOC_SIZE, '{}'",
            transaction_cache_threshold,
            PMEMOBJ_MAX_ALLOC_SIZE
        );
    ...
    fn set_transaction_cache_threshold(&self, cache_threshold: u64) {
        debug_assert!(
            cache_threshold <= PMEMOBJ_MAX_ALLOC_SIZE as u64,
            "cache_threshold '{}' exceeds 0 and PMEMOBJ_MAX_ALLOC_SIZE '{}'",
            cache_threshold,
            PMEMOBJ_MAX_ALLOC_SIZE
        );

        TRANSACTION_CACHE_THRESHOLD_KEY.set_integer(self.0, cache_threshold as i64);
    }
...
static PREFAULT_AT_CREATE_KEY: &'static [u8] = b"prefault.at_create\0";

static PREFAULT_AT_OPEN_KEY: &'static [u8] = b"prefault.at_open\0";

static TRANSACTION_DEBUG_SKIP_EXPENSIVE_CHECKS_KEY: &'static [u8] =
    b"tx.debug.skip_expensive_checks\0";

static TRANSACTION_CACHE_SIZE_KEY: &'static [u8] = b"tx.cache.size\0";

static TRANSACTION_CACHE_THRESHOLD_KEY: &'static [u8] = b"tx.cache.threshold\0";
