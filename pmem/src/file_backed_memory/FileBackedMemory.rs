...
pub trait FileBackedMemory<'memory>: Sized + Send + Sync {
    ...
    const SUPPORTS_EXCLUSIVE_OPEN: bool;

    /// Open file-backed memory from a file.
    #[inline(always)]
    fn open(
        persistent_memory_file_path: &Path,
        exclusive: bool,
    ) -> Result<Option<Self>, PmdkError> {
        if exclusive && !Self::SUPPORTS_EXCLUSIVE_OPEN {
        ...
        const IRRELEVANT_MODE: mode_t = 0;

        Self::_map(persistent_memory_file_path, LENGTH, flags, IRRELEVANT_MODE)
    }
    ...
    fn persist_slowly_at_page_size_granularity(&self, offset: usize, length: usize) {
        ...
        self.offset(offset)
            .persist_or_msync_regardless_of_whether_self_is_persistent_or_regular_memory(length)
    }
    ...
    fn offset(&self, offset: usize) -> *mut c_void {
        debug_assert!(
            offset <= self.mapped_length(),
            "offset '{}' is greater than mapped length '{}'",
            offset,
            self.mapped_length()
        );
    ...
    fn _map(
        persistent_memory_file_path: &Path,
        length: usize,
        open_flags: FileBackedMemoryOpenFlags,
        mode: mode_t,
    ) -> Result<Option<Self>, PmdkError> {
        ...
        if likely(Self::_finish_mapping_if_memory_is_of_correct_type(
            is_persistent_memory,
            address.is_persistent_memory_that_supports_flushing_with_persist(mapped_length),
        )) {
