...
pub trait c_voidExt {
    ...
    #[inline(always)]
    fn persist_or_msync_regardless_of_whether_self_is_persistent_or_regular_memory(
        self,
        length: usize,
    );
...
impl c_voidExt for *mut c_void {
    #[inline(always)]
    fn is_persistent_memory_that_supports_flushing_with_persist(self, length: usize) -> bool {
        (self as *const c_void).is_persistent_memory_that_supports_flushing_with_persist(length)
    }

    #[inline(always)]
    fn persist(self, length: usize) {
        (self as *const c_void).persist(length)
    }
    ...
    #[inline(always)]
    fn flush(self, length: usize) {
        (self as *const c_void).flush(length)
    }
}
