...
#[deprecated(
    note = "Always false, and not useful to know as a fence is still needed, ie calls to drain_after_flush() can not be avoided"
)]
#[inline(always)]
pub fn has_hardware_drain_instruction() -> bool {
    ...
    if likely(result == 0) {
        ...
        panic!(
            "pmem_has_hw_drain() returned value '{}', not 0 or 1",
            result
        );
    }
}
