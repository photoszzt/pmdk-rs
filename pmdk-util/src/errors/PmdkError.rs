...
pub struct PmdkError {
    ...
    pub last_error_message_on_this_thread:
        Result<String, LastErrorMessageOnThisThreadIsInvalidError>,
    ...
    fn fmt(&self, formatter: &mut Formatter) -> Result<(), fmt::Error> {
        ...
        if likely(last_error_message_on_this_thread.is_ok()) {
            write!(
                ...
                self.os_error_number,
                last_error_message_on_this_thread.unwrap()
            )
        } else {
            write!(
                formatter,
                "Generic Error Number '{}': No message ({})",
                self.os_error_number,
                last_error_message_on_this_thread.unwrap_err()
            )
        }
    }
}
...
impl PmdkError {
    #[inline(always)]
    pub fn block<T>(function_name: &'static str) -> Result<T, Self> {
        Self::err(pmemblk_errormsg, function_name)
    }

    #[inline(always)]
    pub fn block_panic(function_name: &'static str) -> ! {
        Self::new(pmemblk_errormsg, function_name).panic()
    }

    #[inline(always)]
    pub fn log<T>(function_name: &'static str) -> Result<T, Self> {
        Self::err(pmemlog_errormsg, function_name)
    }

    #[inline(always)]
    pub fn obj<T>(function_name: &'static str) -> Result<T, Self> {
        Self::err(pmemobj_errormsg, function_name)
    }

    #[inline(always)]
    pub fn pmem<T>(function_name: &'static str) -> Result<T, Self> {
        Self::err(pmem_errormsg, function_name)
    }

    #[inline(always)]
    pub fn err<T>(error_function: ErrorFunction, function_name: &'static str) -> Result<T, Self> {
        Err(Self::new(error_function, function_name))
    }

    #[inline(always)]
    pub fn new(error_function: ErrorFunction, function_name: &'static str) -> Self {
        let os_error_number = errno().0;
        let last_error_message_on_this_thread =
            LastErrorMessageOnThisThreadIsInvalidError::last_error_message_on_this_thread(
                error_function,
            );
        ...
        if likely(os_error_number > 0) {
            ...
            if likely(last_error_message_on_this_thread.is_ok()) {
                panic!(
                    "Invalid errno value '{}' from {} (last error message was '{}')",
                    os_error_number,
                    function_name,
                    last_error_message_on_this_thread.unwrap()
                );
